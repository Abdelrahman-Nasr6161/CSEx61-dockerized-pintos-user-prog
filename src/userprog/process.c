...
char *program_name = strtok_r(file_name, " ", &save_ptr);
    success = load(program_name, &if_.eip, &if_.esp, &save_ptr);
    struct thread *child = thread_current();
    struct thread *parent = child->parent;
    if (success)
    {
        parent->child_loaded = true;
        list_push_back(&parent->children_list, &child->child_list_elem);
        sema_up(&parent->load_wait);
...
int process_wait(tid_t child_tid)
{
    ...
    for (struct list_elem *listElement = list_begin(&parent->children_list); listElement != list_end(&parent->children_list); listElement = list_next(listElement))
    {
        struct thread *child = list_entry(listElement, struct thread, child_list_elem);

        /**
         * The child is found, notify parent then remove it from the parent's children_list list
         * Parent wait for the child to finish execution
        */
        if (child->tid == child_tid)
        {
            list_remove(&child->child_list_elem);
            sema_up(&child->load_wait);
...
void process_exit(void)
{
    ...
    {
        struct used_file *used_file = list_entry(list_pop_back(&cur->files), struct used_file, elem);
        file_close(used_file->ptr);
        palloc_free_page(used_file);
    }

    // End All children_list processes.
    while (!list_empty(&cur->children_list))
    {
        struct thread *child = list_entry(list_pop_back(&cur->children_list), struct thread, child_list_elem);
        sema_up(&child->load_wait);
        
        // Check recursively for children_list of children_list
        sema_down(&child->child_wait);
        
        // Remove Orphan children_list.
        child = NULL;
